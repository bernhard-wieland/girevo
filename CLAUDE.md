...
- **`web/`** — Next.js (App Router, TypeScript, Tailwind). The React app = website + PWA. This is
  ...
  - Shared chrome: `SiteHeader` (sticky translucent) / `SiteFooter` in `layout.tsx`, nav from
    `src/lib/nav.ts` (`headerNav()` = top nav; `footerNav()` + `legalNav()` = footer). Homepage
    hero card asks the one anchoring input (how active you are) → shows the D.1 base band → hands
    off to the real finder; the design's rep-count mini-calc was dropped, same reason as the wizard.
